...
class DraggableNode:
    ...
    def __init__(self, x, y, sigma, xspace = None):
        ...
        self.line = plt.gca().plot(xspace, self.function(xspace), color = 'grey')[0]
    ...
    def connect(self):
        ...
        self.cidpick = self.node.figure.canvas.mpl_connect('pick_event', self.on_pick)        
    def on_press(self, event):
        
        if event.button == 2: return
            
        'on button press we will see if the mouse is over us and store some data'
        if event.inaxes != self.node.axes: return
        if DraggableNode.lock is not None: return #if dragging something else then return
        
        contains, attrd = self.node.contains(event)
        if not contains: return #return if click not in node
        x0, y0 = self.node.get_xdata(), self.node.get_ydata() #x and y of node
        self.press = x0, y0, event.xdata, event.ydata #stores the coordinates of the node and of the event
        ...
        #animate all peaks
            ...
            [widget.line.set_animated(True) for widget in DraggableNode._instances]
            [widget.node.set_animated(True) for widget in DraggableNode._instances]
    ...
    def on_motion(self, event):
        ...
        #set params and draw all peaks
            ...
            self.function[0].params['amp'] = (y0 + dy) - (sum([func(self.node.get_xdata()) for func in DraggableNode.functions]) - self.function[0].params['amp'])
            self.function[0].params['mean'] = x0 + dx
            for widget in DraggableNode._instances:
                ...
                if widget.node != self.node:
                    widget.node.set_ydata(sum([func(widget.node.get_xdata()) for func in DraggableNode.functions]))
            self.line.set_ydata(sum([func(self.xspace) for func in DraggableNode.functions]))
    ...
    def on_pick(self, event):
        if event.artist != self.node: return
        self.press = self.node.get_xdata(), self.node.get_ydata(), self.node.get_xdata(), self.node.get_ydata()
    ...
    def on_release(self, event):
        ...
        if DraggableNode.composite == True:
            [widget.node.set_animated(False) for widget in DraggableNode._instances]
    ...
    def on_key(self, event):
        if event.key == " ":
            ...
            if DraggableNode.composite == False:
                ...
                [widget.line.set_color("grey") for widget in DraggableNode._instances]
            if DraggableNode.composite == True:
                [widget.line.set_ydata(None) for widget in DraggableNode._instances]
                DraggableNode._instances[-1].line.set_ydata( sum([ widget.function(widget.xspace) for widget in DraggableNode._instances]) )
                for widget in DraggableNode._instances:
                    y = sum(wid.function(widget.node.get_xdata()) for wid in DraggableNode._instances)
                    widget.node.set_ydata(y[0])
                    widget.node.set_animated(False)
                    
                [widget.line.set_color('C0') for widget in DraggableNode._instances]
            ...
            self.ax.cla()
            [self.ax.plot(self.xspace, func(self.xspace), linestyle = ':') for func in DraggableNode.functions]
            ...
            if showparams:
                ...
                for func in function[1:]:
                    plt.vlines(x = func.params['mean'], ymin = -500, ymax = func.params['amp'], color = 'grey', linestyle = ':')
                    mean = func.params['mean']
                    amp = func.params['amp']
                    std = func.params['sigma']
                    spacing = (ylim[1] - ylim[0])/15
                    texts = [plt.text(mean, amp + spacing, "A: {:.2f} \nμ: {:.2f}\nσ: {:.2f}".format(amp, mean, std), fontsize = 8, horizontalalignment='center', weight = 'bold')]
